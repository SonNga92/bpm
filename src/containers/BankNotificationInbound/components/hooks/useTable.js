...
import bankNotificationInboundApi from '../../../../api/bankNotificationInboundApi';
...
const useTable = () => {
  ...
  const getTableData = useCallback(async (params, searchValues) => {
    try {
      ...
      let response = {};
      searchValues
        ? (response = await bankNotificationInboundApi.search({
            ...params,
            ...searchValues
          }))
        : (response = await bankNotificationInboundApi.getAll(params));
      const { data } = response;
      let rows = [];
      if (data) {
  ...
  const onDelete = async (params) => {
    try {
      if (params._id) {
        await bankNotificationInboundApi.delete(params);
      }
    } catch (error) {
  ...
  const onAdd = async (params) => {
    try {
      if (params) {
        bankNotificationInboundApi.add(params);
  ...
  const onEdit = async (params) => {
    try {
      if (params && editId) {
        bankNotificationInboundApi.edit(params, editId);
        setTimeout(() => getTableData(), 300);
      }
    } catch (error) {
      console.log('Failed to add user', error);
