...
import bankTransactionResponseApi from '../../../../api/bankTransactionResponseApi';
...
const useTable = () => {
  ...
  const getTableData = useCallback(async (params, searchValues) => {
    try {
      ...
      let response = {};
      searchValues
        ? (response = await bankTransactionResponseApi.search({
            ...params,
            ...searchValues
          }))
        : (response = await bankTransactionResponseApi.getAll(params));
      const { data } = response;
      let rows = [];
      if (data) {
  ...
  const onDelete = async (params) => {
    try {
      if (params._id) {
        await bankTransactionResponseApi.delete(params);
      }
    } catch (error) {
  ...
  const onAdd = async (params) => {
    try {
      if (params) {
        bankTransactionResponseApi.add(params);
  ...
  const onEdit = async (params) => {
    try {
      if (params && editId) {
        bankTransactionResponseApi.edit(params, editId);
        setTimeout(() => getTableData(), 300);
      }
    } catch (error) {
      console.log('Failed to add user', error);
